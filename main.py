...
flags.DEFINE_integer("l1_remove_epoch", 150, "Epoch where L1 in G is "
                                           "removed (Def: 150).")
flags.DEFINE_integer("baseline_remove_epoch", 50, "Epoch where baseline in G is "
                                           "removed (Def: 50).")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        if FLAGS.test_wav is None:
            ...
            if not os.path.isdir(FLAGS.test_wav):
                ...
                wavFiles = glob.glob(os.path.join(FLAGS.test_wav,'*.wav'))
                for wavFile in wavFiles:
                    fm, wav_data = wavfile.read(wavFile)
                    wavname = wavFile.split('/')[-1]
                    if fm != 16000:
                        raise ValueError('16kHz required! Test file is different')
                    wave = (2./65535.) * (wav_data.astype(np.float32) - 32767) + 1.
                    print('test wave shape: ', wave.shape)
