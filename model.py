...
class SEGAN(Model):
    ...
    def __init__(self, sess, args, devices, infer=False, name='SEGAN'):
        ...
        self.disc_updates = 1
        # do extra interation in generator for comparing the fake result with pre-enhanced result
        self.gen_comp_baseline = False
        self.gen_comp_baseline = tf.Variable(self.gen_comp_baseline, trainable=False)
        # canvas size
    ...
    def build_model_single_gpu(self, gpu_idx):
        if gpu_idx == 0:
            ...
            self.get_wav, self.get_noisy, self.get_baseline = read_and_decode(filename_queue,
                                                           2 ** 14)
        # load the data to input pipeline
        wavbatch, \
        noisybatch, \
        baselinebatch  = tf.train.shuffle_batch([self.get_wav,
                                             self.get_noisy,
                                             self.get_baseline],
            ...
            while not coord.should_stop():
                ...
                if counter % config.save_freq == 0:
                    ...
                    g_l1_loss = self.sess.run([g_opt, self.g_sum,
                                               self.g_adv_losses[0],
                                               self.g_l1_losses[0]])
                    g_l1_baseline_loss = 0
                    g_adv_baseline_loss = 0
                    if curr_epoch < self.args.baseline_remove_epoch:
                        self.sess.run(tf.assign(self.gen_comp_baseline, True))
                        _g_opt, _g_sum2, \
                        g_adv_baseline_loss, \
                        g_l1_baseline_loss = self.sess.run([g_opt, self.g_sum,
                                                    self.g_adv_losses[0],
                                                   self.g_l1_losses[0]])
                        self.sess.run(tf.assign(self.gen_comp_baseline, False))
                    ...
                    if curr_epoch < self.args.baseline_remove_epoch:
                        ...
                        _g_opt,  \
                        g_adv_baseline_loss, \
                        g_l1_baseline_loss = self.sess.run([g_opt, self.g_adv_losses[0],
                                                   self.g_l1_losses[0]])
                        self.sess.run(tf.assign(self.gen_comp_baseline, False))
                ...
                print('{}/{} (epoch {}), d_rl_loss = {:.5f}, '
                      ...
                      'g_adv_loss = {:.5f}, g_l1_loss = {:.5f}, g_adv_baseline_loss = {:.5f}, g_l1_baseline_loss = {:.5f},'
                      ' time/batch = {:.5f}, '
